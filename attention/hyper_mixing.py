...
from fast_transformers.attention_registry import AttentionRegistry, Int, Choice, Float, Bool, EventDispatcherInstance
from fast_transformers.attention_registry import Optional as Opt
from fast_transformers.events import EventDispatcher, AttentionEvent
from fast_transformers.masking import BaseMask
...
class HyperMixing(nn.Module):
    ...
    def __init__(
        ...
        n_heads: int,
        hypernet_size: Optional[int] = None,
        hypernet_tied: bool = False,
        token_information: Optional[Callable[[torch.Tensor], torch.Tensor]] = None,
        hypernet_layer_norm: bool = True,
        hypernet_bias: bool = False,
    ) -> None:
        ...
        hypernet_layer_norm : bool
            If True, layer normalization is used.
        """
        super().__init__()
        self.input_output_dim = n_heads * query_dimensions
        self.hypernetwork_bias = hypernet_bias
        self.hyper = _HyperNetwork(
            n_heads * query_dimensions,
            hypernet_size,
            tied=hypernet_tied,
            num_heads=n_heads,
            keep_output_size=False,
            hypernetwork_bias=hypernet_bias,
        )
        self.activation = nn.GELU()
        self.num_heads = n_heads
        self.use_layer_norm = hypernet_layer_norm
        if self.use_layer_norm:
            self.layer_norm = nn.LayerNorm(n_heads * query_dimensions)

        # add pos encoding or a custom token_information function
        if token_information is None:
            self.token_information = _PositionalEncoding(n_heads * query_dimensions)
        else:
    ...
    def forward(
        ...
        key: torch.Tensor,
        value: torch.Tensor,
        attn_mask: Optional[BaseMask] = None,
        query_padding_mask: Optional[BaseMask] = None,
        key_padding_mask: Optional[BaseMask] = None,
    ):
        """
        ...
        query = query.view(bsize, query_len, -1)
        key = key.view(bsize, key_len, -1)
        value = value.view(bsize, key_len, -1)

        if value.size(1) != key.size(1):
            raise ValueError("Number of values not equal to number of keys!")

        if key_padding_mask is not None:
            key_float_mask = key_padding_mask.float_matrix[:, :, None]
            key = key * key_float_mask
            value = value * key_float_mask
        if query_padding_mask is not None:
            query_float_mask = query_padding_mask.float_matrix[:, :, None]
        ...
        if attn_mask is not None and attn_mask.lower_triangular:
            out = _causal_mlp_pass_from_components(value, W1, W2, self.activation)
        else:
...
AttentionRegistry.register(
    "hyper_mixing", HyperMixing,
    [
        ("query_dimensions", Int),
        ("n_heads", Int),
        ("hypernet_size", Opt(Int)),
        ("hypernet_tied", Opt(Bool, False)),
        ("hypernet_layer_norm", Opt(Bool, True)),
        ("hypernet_bias", Opt(Bool, False)),
    ]
)
...
def _causal_mlp_pass_from_components(
    out, W1: torch.Tensor, W2: torch.Tensor, activation: nn.Module
) -> torch.Tensor:
    # out: [B d, T1]
    # W1: [B, T1, d]
    ...
    for i in range(0, out.size(-1)):
        if i == 0:
            res = _mlp_pass_from_components(out[:, :, :i+1], W1[:, :i+1, :], W2[:, :i+1, :], activation)
        else:
            res = torch.cat(
                (res, _mlp_pass_from_components(out[:, :, :i+1], W1[:,:i+1, :], W2[:, i:i+1, :], activation)),
                -1
            )
    return res
...
class _PositionalEncoding(nn.Module):
    ...
    def __init__(self, d_model: int, max_seq_len: int = 5001) -> None:
        super().__init__()
